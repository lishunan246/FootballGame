...
public class Ball_Spawn_Point : MonoBehaviour {
	...
	void ShootBall()
	{
		player.gameObject.GetComponent<Animation>().Play("pass");
		kickedBall = true;
		shotsLeft -= 1;
		...
		ball.GetComponent<Rigidbody>().AddForce(transform.forward * shotPower, ForceMode.Impulse);
		ball.GetComponent<Rigidbody>().AddForce(Vector3.right * Random.Range(1,7), ForceMode.Impulse); //add random force right
		ball.GetComponent<Rigidbody>().AddForce(Vector3.left * Random.Range(1,7), ForceMode.Impulse); //add random force left
	}
	...
	void Update()
	{
		...
		if(!kickedBall)
		{
			player.gameObject.GetComponent<Animation>().Play("defenderIdle");
		}
