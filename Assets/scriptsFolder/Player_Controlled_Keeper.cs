...
/*
 * This script is attached to the goalkeeper in the Goalkeeper Mode level.
 */ 

public class Player_Controlled_Keeper : MonoBehaviour {
	
	public string Name;	
	
	/*
	 * This allows me to control the different states of the goalkeeper
	 */ 
	...
	void Update () {
		...
		case GoalKeeper_State.JUMP_RIGHT:
			...
			{
				transform.position += transform.right * Time.deltaTime * 8.0f;
			}		
		...
		case GoalKeeper_State.RESTING:
			...
			if ( distanceBall < 10.0f ) {
				state = Player_Controlled_Keeper.GoalKeeper_State.RESTING;
				//come back to thus
			} 
			break;
		}
		...
		{
			state = GoalKeeper_State.JUMP_RIGHT;
		}
